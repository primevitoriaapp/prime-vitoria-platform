...
export function ClientPassengersPanel({ trips }: Props) {
  ...
  return (
    <section id="passageiros" className="space-y-3">
      <h2 className="font-serif text-lg text-white">Passageiros</h2>
      <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
        {passengers.length === 0 ? (
          <p className="text-sm text-slate-500">Nenhum passageiro nas corridas listadas.</p>
        ) : (
      ...
      <p className="text-xs text-slate-600">Agregado a partir do histórico de viagens (sem cadastro separado).</p>
    </section>
  );
}
