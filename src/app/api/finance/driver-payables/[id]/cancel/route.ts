...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (row.status === "cancelled") return ok({ id: payableId, status: "cancelled", already: true });
    if (row.status === "paid") return fail("PAYABLE_PAID", "Estorne o pagamento antes de cancelar", 409);
    if (row.status !== "open") return fail("INVALID_STATUS", `Estado: ${row.status}`, 409);
    ...
    await insertAuditEvent({
      ...
      action: "finance.driver_payable_cancelled",
    ...
    return ok({ id: payableId, status: "cancelled" });
  } catch (error) {
