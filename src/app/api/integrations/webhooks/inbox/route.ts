import { z } from "zod";
import { db } from "@/lib/server/db";
import { fail, mapApiError, ok } from "@/lib/server/http";
import { getSessionContext } from "@/lib/server/session";
import { assertTenantScope } from "@/lib/server/tenant-scope";
import { assertCapability } from "@/lib/security/rbac";
import { runIntegrationGuards } from "@/lib/security/integration-guard";

const listSchema = z.object({
  status: z.enum(["pending", "processed", "ignored", "error"]).optional(),
  provider: z.enum(["omie", "conta_azul", "generic"]).optional(),
  page: z.coerce.number().int().min(1).default(1),
  pageSize: z.coerce.number().int().min(1).max(100).default(50)
});

function mapIntegrationError(error: unknown) {
  const message = error instanceof Error ? error.message : String(error);
  if (message.includes("Integration access denied")) return fail("FORBIDDEN_IP", message, 403);
  if (message.includes("Rate limit exceeded")) return fail("RATE_LIMIT", message, 429);
  if (message.startsWith("Forbidden:")) return fail("FORBIDDEN", message.replace(/^Forbidden:\s*/, ""), 403);
  return mapApiError(error);
}

/** Lista webhooks ERP recebidos do tenant (caixa de entrada). */
export async function GET(request: Request) {
  try {
    runIntegrationGuards(request, "webhook-inbox-get");
    const session = await getSessionContext();
    assertCapability(session, "erp.mapping.read");
    const tenantId = assertTenantScope(session);

    const q = listSchema.parse(Object.fromEntries(new URL(request.url).searchParams.entries()));
    const from = (q.page - 1) * q.pageSize;
    const to = from + q.pageSize - 1;

    let query = db
      .from("erp_webhook_inbox")
      .select(
        "id, provider, status, received_at, processed_at, last_error, attempt_count, payload",
        { count: "exact" }
      )
      .eq("tenant_id", tenantId)
      .order("received_at", { ascending: false })
      .range(from, to);

    if (q.status) query = query.eq("status", q.status);
    if (q.provider) query = query.eq("provider", q.provider);

    const { data, error, count } = await query;
    if (error) return fail("WEBHOOK_INBOX_LIST_FAILED", error.message, 500);

    const items = (data ?? []).map((row) => {
      const payload = row.payload;
      const payloadPreview =
        payload == null
          ? null
          : typeof payload === "string"
            ? payload.slice(0, 400)
            : JSON.stringify(payload).slice(0, 400);
      return {
        id: row.id,
        provider: row.provider,
        status: row.status,
        received_at: row.received_at,
        processed_at: row.processed_at,
        last_error: row.last_error,
        attempt_count: row.attempt_count,
        payload_preview: payloadPreview
      };
    });

    return ok({
      items,
      page: q.page,
      pageSize: q.pageSize,
      total: count ?? 0
    });
  } catch (error) {
    return mapIntegrationError(error);
  }
}
