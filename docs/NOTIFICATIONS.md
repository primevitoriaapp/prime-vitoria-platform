...
- Operacional (`operations.*`) para `admin` e `operador`: nova solicitacao, corrida aprovada, claim assumido por outro, **despacho com motorista** (`operations.trip_dispatched`), **reatribuicao** (`operations.trip_reassigned`), **cancelamento** (`operations.trip_cancelled`), **deslocamento** (`operations.trip_on_the_way`) e **chegada** (`operations.trip_arrived`) — ver `operational-notify.ts`
