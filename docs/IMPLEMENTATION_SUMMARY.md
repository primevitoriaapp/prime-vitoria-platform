...
- **In-app status operacional:** transições para cancelada, em deslocamento e no local notificam `admin`/`operador` via `operations.trip_cancelled`, `operations.trip_on_the_way` e `operations.trip_arrived`.
...
- **Hardening smoke HTTP/preflight:** `scripts/e2e-smoke-http.mjs` e `scripts/vercel-preflight.mjs` detectam Deployment Protection da Vercel, aceitam `VERCEL_AUTOMATION_BYPASS_SECRET` para validar deployment protegido e falham com diagnóstico accionável, evitando falso `200 OK` na página de login da Vercel.
