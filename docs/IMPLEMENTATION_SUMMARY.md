...
- **Hardening smoke HTTP/preflight:** `scripts/e2e-smoke-http.mjs`, `scripts/vercel-preflight.mjs` e `scripts/go-live-preflight.mjs` usam loader comum de envs (`.env.supabase.local`, `.env.vercel.local`, `.env.local`, `.env`), detectam Deployment Protection da Vercel, aceitam `VERCEL_AUTOMATION_BYPASS_SECRET` para validar deployment protegido e falham com diagnóstico accionável, evitando falso `200 OK`/HTML na página de login da Vercel.
