#!/usr/bin/env bash
# Aplica migration 0045 APENAS em staging (foto motorista + bucket storage).
#
# Uso:
#   export STAGING_DATABASE_URL="postgresql://..."
#   export CONFIRM_STAGING_MIGRATION=true
#   npm run db:apply-0045-staging
#
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SQL_FILE="$ROOT/supabase/migrations/0045_driver_photo_url.sql"
...
echo "=== Apply migration 0045 (staging) ==="
...
VALUES ('0045', 'driver_photo_url', ARRAY[]::text[])
ON CONFLICT (version) DO NOTHING;
EOSQL

echo ""
echo "OK: migration 0045 aplicada. Validar:"
